...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator, operands = scheme_eval(first, env), rest.map(lambda element: scheme_eval(element, env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if vals.__len__() != formals.__len__():
            raise SchemeError()
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        name, formals, body = expressions.first.first, expressions.first.rest, expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        evaluated_val, formals = scheme_eval(bindings.first.rest.first, env), Pair(bindings.first.first, formals)

        check_formals(formals)

        vals, bindings = Pair(evaluated_val, vals), bindings.rest
