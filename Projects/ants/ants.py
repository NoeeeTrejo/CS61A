...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None:
            return False
        elif self.place.ant is not None and not self.place.ant.blocks_path:
            return False
...
class HarvesterAnt(Ant):
    """HarvesterAnt produces 1 additional food per turn for the c5olony."""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            if current_place.bees and current_distance <= self.max_range and current_distance >= self.min_range:
    ...
    def throw_at(self, target):
        """Throw a leaf at the TARGET , reducing its armor."""
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        print("DEBUG: Digesting = ", self.digesting)

        if self.digesting == 0 and len(self.place.bees):
            self.eat_bee(random_or_none(self.place.bees))
            self.digesting = self.time_to_digest

        elif self.digesting > 0 and len(self.place.bees):
            self.digesting -= 1
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        bees_that_pass = self.place.bees[:]
        for bee in bees_that_pass:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        #### It does the same thing as hte bodyguard ants action but it also
        #### does damage to the bees so we can call BodyguardAnt's action
        # if self.contained_ant:
        #     self.contained_ant.action(colony)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor)
        self.imposter = QueenAnt.no_queen
        if self.imposter == False:
            QueenAnt.no_queen = True
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while place:
                if place.ant:
                    if (place.ant not in QueenAnt.damaged_ants):
                        place.ant.damage = 2 * place.ant.damage
                        QueenAnt.damaged_ants.append(place.ant)

                    if place.ant.is_container and place.ant.contained_ant and place.ant.contained_ant not in QueenAnt.damaged_ants:
                        place.ant.contained_ant.damage = 2 * place.ant.contained_ant.damage
                        QueenAnt.damaged_ants.append(place.ant.contained_ant)
    ...
    def reduce_armor(self, amount):
        """Reduce armor by AMOUNT, and if th
        e True QueenAnt has no armor
        remaining, signal the end of the game.
        """
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        # if self.armor <= 0 or self.place.exit_______: #a bee reaches the end

        #     bees_win()
        ThrowerAnt.reduce_armor(self, self.armor)
